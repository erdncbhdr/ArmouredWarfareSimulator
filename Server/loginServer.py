...
def allXP(self, username):
    ...
    xp = LogServer.cur.execute(
        "SELECT * FROM UserProgress INNER JOIN UserInfo ON UserInfo.UserId = UserProgress.UserId WHERE"
...
def owned(self, username):
    ...
    owned = LogServer.cur.execute(
        "SELECT * FROM UserOwned INNER JOIN UserInfo ON UserOwned.UserId = UserInfo.UserId WHERE "
